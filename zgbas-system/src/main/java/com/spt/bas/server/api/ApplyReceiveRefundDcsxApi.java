...
import com.spt.bas.server.service.IApplyReceiveRefundDcsxService;
...
public class ApplyReceiveRefundDcsxApi extends BaseApi<ApplyReceiveRefundDcsx> {
	@Autowired
	...
	@Override
	public IBaseService<ApplyReceiveRefundDcsx> getService() {
		return applyReceiveRefundDcsxService;
	}
	
	@PostMapping("updateFileId")
	public void updateFileId(@RequestBody FileIdUpdateVo vo) {
		applyReceiveRefundDcsxService.updateFileId(vo.getId(), vo.getFileId());
	}
	
	@PostMapping("findPageDetail")
	public Page<ApplyCancelDetail> findPageDetail(@RequestBody PageSearchVo searchVo){
		Page<ApplyReceiveRefundDcsx> page = applyReceiveRefundDcsxService.findPage(searchVo);
		List<ApplyCancelDetail> list = new ArrayList<ApplyCancelDetail>();
		for (ApplyReceiveRefundDcsx out : page.getContent()) {
