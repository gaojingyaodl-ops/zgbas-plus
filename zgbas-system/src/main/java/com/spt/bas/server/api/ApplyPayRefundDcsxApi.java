...
@RestController
@RequestMapping(value = "apply/refundPayDcsx")
public class ApplyPayRefundDcsxApi extends BaseApi<ApplyPayRefundDcsx> {
	@Autowired
	...
	@Override
	public IBaseService<ApplyPayRefundDcsx> getService() {
		return applyPayRefundDcsxService;
	}
	
	@PostMapping("updateFileId")
	public void updateFileId(@RequestBody FileIdUpdateVo vo) {
		applyPayRefundDcsxService.updateFileId(vo.getId(), vo.getFileId());
	}
	
	@PostMapping("findPageDetail")
	public Page<ApplyCancelDetail> findPageDetail(@RequestBody PageSearchVo searchVo){
		Page<ApplyPayRefundDcsx> page = applyPayRefundDcsxService.findPage(searchVo);
		List<ApplyCancelDetail> list = new ArrayList<ApplyCancelDetail>();
		for (ApplyPayRefundDcsx out : page.getContent()) {
