...
public class BsCompanyManageServiceImplBak extends BaseService<BsCompany> {
	@Autowired
	 ...
	 *   b. 释放给区域总，区域总可以指派，不能分享
	 *   c. 如果该客户已经被连续三次释放给区域总后，则直接释放到公海
	 *
	 */
	public void updateStatusByTask(String companyName) {
		...
		Map<String, BsDictData> releaseDeptMap = new HashMap<>();
		if (CollectionUtils.isNotEmpty(dictDataList)) {
			releaseDeptMap = dictDataList.stream()
		...
		for (BsCompany bc : companyList) {
			Map<Long, SysDeptSdk> finalLeaderDeptAllMap = leaderDeptAllMap;
			Map<String, BsDictData> finalReleaseDeptMap = releaseDeptMap;
			Future<Void> future = executorService.submit(() -> {
				try {
					processCompany(bc, nowDate, own, notOwn, finalLeaderDeptAllMap, updateStep, compareDate, listByCategory, finalReleaseDeptMap);
					return null;
				} catch (Exception e) {
	...
	@Transactional
	public void processCompany(BsCompany bc, Date nowDate, Long own, Long notOwn, Map<Long,SysDeptSdk> leaderDeptAllMap,
								   String updateStep, Date compareDate,List<BsDictData> listByCategory,Map<String, BsDictData> releaseDeptMap) {
		long count = 0;
		long count1 = 0;
		...
		// 人保是否已批复
		Boolean piccApproveSuccess = false;
		String piccApplyStatus = bc.getPiccApplyStatus();
		if (StringUtils.equals(BasConstants.PICC_APPLY_STATUS_1, piccApplyStatus)) {
			return;
		} else if (StringUtils.equals(BasConstants.PICC_APPLY_STATUS_2, piccApplyStatus)) {
			piccApproveSuccess = true;
		}

		Date piccApproveDate = bc.getPiccApproveDate();
		Date matchFllowDate = bc.getMatchFllowDate();
		Date matchFllowDateCompare = bc.getMatchFllowDate();
		if (piccApproveSuccess) {
			if (Objects.isNull(piccApproveDate)) {
				matchFllowDateCompare = matchFllowDate;
			} else if (Objects.isNull(matchFllowDate)) {
				matchFllowDateCompare = piccApproveDate;
			} else {
				if (piccApproveDate.compareTo(matchFllowDate) > 0) {
					matchFllowDateCompare = piccApproveDate;
				} else {
					matchFllowDateCompare = matchFllowDate;
				}
		...
		if (!departFlg) {
			...
			} else if (piccApproveSuccess && Objects.nonNull(piccApproveDate) && DateUtil.between(piccApproveDate, nowDate, DateUnit.DAY) <= notOwn){
				// 人保批复日期
				return;
			}
		...
		if (updateFlg && CollectionUtil.isEmpty(collect)) {
			releaseCompany(bc,freedToDeptLeaderCount,updateStep,ctrContracts,compareDate,matchUserId,leaderFlg,matchUser,releaseDeptMap);
		} else {
			// 有成交记录，释放给区域总次数置为0
	 ...
	 */
	@Transactional
	public void releaseCompany(BsCompany bc, Integer freedToDeptLeaderCount, String updateStep,List<CtrContract> ctrContracts,
							   Date compareDate, Long matchUserId, Boolean leaderFlg, SysUserSdk matchUser, Map<String, BsDictData> releaseDeptMap) {
		...
		if (Objects.nonNull(ctrContract) && DateOperator.compareDays(compareDate, ctrContract.getContractTime()) >= 0) {
			...
			if (Objects.nonNull(matchUser)) {
				// 获取需要释放给区域总的部门
				BsDictData bsDictData = releaseDeptMap.get(matchUser.getDeptId() + "");
				// 业务员部门在字典配置需释放给区域总的部门列表中，则需要优先释放给区域总否则直接释放到公海
				if (Objects.nonNull(bsDictData)) {
					toLeaderFlg = true;
				}
			}
			// 该员工所在的部门是否需要释放给区域总
			if (toLeaderFlg && freedToDeptLeaderCount < 3) {
				// 判断是否已释放给区域总三次，是：释放到公海，否：释放给区域总
				if (leaderFlg) {
					// 判断当前是否为区域总，是区域总则不往上释放
					vo.setStatus(BasConstants.COMPANY_STATUS_F);
					vo.setCreateUserId(matchUserId);
					handelDeptId(vo, matchUser);
				} else {
					...
					if (leaderId != null) {
						SysUserSdk leaderUser = authOpenFacade.findUserById(leaderId);
						if (Objects.nonNull(leaderUser)) {
							vo.setStatus(BasConstants.COMPANY_STATUS_F);
							vo.setCreateUserId(leaderUser.getUserId());
							handelDeptId(vo, leaderUser);
