...
public class ApplyReceiveRefundApi extends BaseApi<ApplyReceiveRefund> {
	@Autowired
	private IApplyReceiveRefundService applyReceiveRefundService;
	@Autowired
	private IPmApproveService pmApproveService;
	
	@Override
	public IBaseService<ApplyReceiveRefund> getService() {
		return applyReceiveRefundService;
	}
	
	@PostMapping("updateFileId")
	public void updateFileId(@RequestBody FileIdUpdateVo vo) {
		applyReceiveRefundService.updateFileId(vo.getId(), vo.getFileId());
	}
	
	@PostMapping("findPageDetail")
	public Page<ApplyCancelDetail> findPageDetail(@RequestBody PageSearchVo searchVo){
		Page<ApplyReceiveRefund> page = applyReceiveRefundService.findPage(searchVo);
		List<ApplyCancelDetail> list = new ArrayList<ApplyCancelDetail>();
		for (ApplyReceiveRefund out : page.getContent()) {
