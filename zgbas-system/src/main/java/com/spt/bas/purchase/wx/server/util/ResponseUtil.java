...
public class ResponseUtil {
    ...
    public static void renderJson(HttpServletResponse response, Status status, Object data) {
        try {
            ...
            response.getWriter()
                    .write(JSONUtil.toJsonStr(new JSONObject(ApiResult.ofStatus(status, data), false)));
        } catch (IOException e) {
    ...
    public static void renderJson(HttpServletResponse response, BaseException exception) {
        try {
            ...
            //  将JSON转为String的时候，忽略null值的时候转成的String存在错误
            response.getWriter()
                    .write(JSONUtil.toJsonStr(new JSONObject(ApiResult.ofException(exception), false)));
        } catch (IOException e) {
            log.error("Response写出JSON异常，", e);
        }
    }
