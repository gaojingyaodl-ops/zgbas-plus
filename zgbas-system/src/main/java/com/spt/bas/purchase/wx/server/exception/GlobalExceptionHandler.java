 ...
 * @Author shengong
 * @Date 2020-07-23 21:54
 * @Description 全局异常处理
 */
@ControllerAdvice(basePackages = "com.spt.bas.purchase.wx.server")
@Slf4j
public class GlobalExceptionHandler {
    ...
    public ApiResult handlerException(Exception e) {
        if (e instanceof NoHandlerFoundException) {
            ...
            return ApiResult.ofStatus(Status.REQUEST_NOT_FOUND);
            ...
            return ApiResult.ofStatus(Status.HTTP_BAD_METHOD);
        } else if (e instanceof MethodArgumentNotValidException) {
            log.error("【全局异常拦截】MethodArgumentNotValidException", e);
            return ApiResult.of(Status.BAD_REQUEST.getCode(), ((MethodArgumentNotValidException) e).getBindingResult()
                    .getAllErrors()
                    .get(0)
                    .getDefaultMessage(), null);
        }else if (e instanceof ConstraintViolationException) {
            log.error("【全局异常拦截】ConstraintViolationException", e);
            return ApiResult.of(Status.BAD_REQUEST.getCode(), CollUtil.getFirst(((ConstraintViolationException) e).getConstraintViolations())
                    .getMessage(), null);
            ...
            return ApiResult.ofStatus(Status.PARAM_NOT_MATCH);
        }else if (e instanceof HttpMessageNotReadableException) {
            log.error("【全局异常拦截】HttpMessageNotReadableException: 错误信息 {}", ((HttpMessageNotReadableException) e).getMessage());
            return ApiResult.ofStatus(Status.PARAM_NOT_NULL);
        } else if (e instanceof ApplicationException) {
            log.error("【全局异常拦截】HttpMessageNotReadableException: 错误信息 {}", ((ApplicationException) e).getMessage());
            return ApiResult.ofStatus(Status.APPLICATION_EXCEPTION);
        } else if (e instanceof UserNameOrPasswordException) {
            log.error("【全局异常拦截】BaseException: 错误信息 {}", e.getMessage());
            return ApiResult.ofStatus(Status.USERNAME_PASSWORD_ERROR);
        } else if (e instanceof BaseException) {
            log.error("【全局异常拦截】：异常信息 {}", e);
            return ApiResult.ofException((BaseException)e);
        }
