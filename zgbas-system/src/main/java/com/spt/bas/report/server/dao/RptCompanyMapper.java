package com.spt.bas.report.server.dao;

import com.spt.bas.report.client.entity.RptCompany;
import com.spt.bas.report.client.entity.RptCompanyToProduct;
import com.spt.bas.report.client.vo.RptCompanySearchVo;
import com.spt.tools.mybatis.annotation.MyBatisDao;

import java.util.List;

@MyBatisDao
public interface RptCompanyMapper{
	
	List<RptCompany> findRptCompanyList(RptCompanySearchVo vo);
	
	List<RptCompany> findRptCompanyListNew(RptCompanySearchVo vo);
	
	List<RptCompanyToProduct> findRptCompanyToProductList();
	
	List<RptCompanyToProduct> findRptLegalList();

}
