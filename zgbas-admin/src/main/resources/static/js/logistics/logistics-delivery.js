var logisticsDelivery={
	addDeliveryTab:function(title, deleteFlg, id, logisticsCount) {
		return new Promise(function(resolve, reject) {
			...
			$.post(url, { curNumber: _curNumber, logisticsId: id, logisticsCount: logisticsCount }, function(html) {
				...
				logisticsDelivery.initLoadEasyui(_curNumber, id, deliveryId);
	 ...
	 addDeliveryTabOld:async function(title,deleteFlg,id,logisticsCount){
		...
		$.post(url,{curNumber:_curNumber,logisticsId:id,logisticsCount:logisticsCount},function(html){
			...
			//加载html的组件
			logisticsDelivery.initLoadEasyui(_curNumber,id,deliveryId);
			// logisticsDelivery.initLoadEasyui(_curNumber);
			$('.easyui-linkbutton').linkbutton();
	...
	initLoadEasyui:async function(randomNum,logisticsId,logisticsDeliveryId){
		...
		var _curColumns = [ [
			...
			{field:'driverCardNo',title:'司机身份证', width:100,editor: { type: 'textbox', options: { required: true}}},
			{
				...
				formatter: function (value, row, index) {
					var str = '';
					if (index == 0) {
						str = '<i class="icon-add icon cursor '+operatorClass+' mt4" style="margin-right: 26px;" onclick="logisticsDelivery.insertLogisticsDeliveryRow(\'' + randomNum + '\')"></i>'
					} else {
						str = '<i class="icon-add icon cursor '+operatorClass+' mt4" onclick="logisticsDelivery.insertLogisticsDeliveryRow(\'' + randomNum + '\')"></i>' +
							'<i class="icon-remove ml10 icon cursor mt4" onclick="logisticsDelivery.removeLogisticsDeliveryRow(\'' + row.id + '\',\'' + index + '\',event,\'' + randomNum + '\')"></i>'
					}
					return str;
				}
			}
		...
		await $('#relaTable'+randomNum).datagrid({ 
			...
			onLoadSuccess: function(data) {
				// logisticsDelivery.insertLogisticsDeliveryRow(randomNum)
				// 数据加载完成后的处理逻辑
		...
		$("#phoneProtect"+randomNum).combobox({
			data:_phoneProtectJson,
			textField:'dictName',
			valueField:'dictCd',
			panelHeight:"auto",
			editable:false,
			...
			onLoadSuccess:function(){
				var phoneProtectVal = $("#phoneProtect"+randomNum).val();
				if(!phoneProtectVal){
					$("#phoneProtect"+randomNum).combobox('setValue','1');
				}
			}
		});
		var isFirstLoad = true; // 初始设定为首次加载
		$("#carrierId"+randomNum).combobox({
			data:_carrierListJson,
			textField:'carrier',
			valueField:'id',
			panelHeight:"auto",
			editable:true,
			...
			panelMaxHeight: 400,
			onSelect:function(data){
				$('#carrier'+randomNum).val(data.carrier);
				// 首次加载不执行后续操作
				if (isFirstLoad) {
					isFirstLoad = false;
					return;
				}
				$("#masterPorter"+randomNum).textbox('setValue',data.contactName);
				$("#masterPhone"+randomNum).textbox('setValue',data.contactPhone);
			},
			onLoadSuccess:function(){
				isFirstLoad = false;
			}
		...
		var realDeliveryDate = $("#realDeliveryDate"+randomNum).val();
		if(!realDeliveryDate) {
			$("#realDeliveryDate"+randomNum).datebox('setValue', _buyDeliveryDate);
		}
		var realArrivalDate = $("#realArrivalDate"+randomNum).val();
		if(!realArrivalDate) {
			$("#realArrivalDate"+randomNum).datebox('setValue', _sellDeliveryDate);
		}
	...
	removeLogisticsDeliveryRow: function (id, _index, event, randomNum) {
		...
		$(operators[operators.length - 1]).removeClass('icon-add-no').addClass('icon-add').attr('onClick', 'logisticsDelivery.insertLogisticsDeliveryRow(\''+randomNum+'\')');
		if (operators.length > 1) {
			$(operators[operators.length - 1]).next().addClass('icon-remove').attr('onClick', 'logisticsDelivery.removeLogisticsDeliveryRow(\'' + 0 + '\',\'' + (_index - 1) + '\',event,\''+randomNum+'\')');
		}
