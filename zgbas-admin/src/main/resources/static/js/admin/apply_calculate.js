		...
		curOper:function(curIndex,row,table){
			...
			var sumAmount=function(){
				...
				if(_dealPrice &&_dealNumber){
					$(_ed_dealAmount.target).numberbox('setValue', Math.round(_dealPrice*_dealNumber*100)/100);
				}
				//计算差价 和毛利润
...
//								console.log("removeArr:"+removelist);
		...
		initLoadEasyui:function(randomNum,type,tag,applyType){
			...
			var _payModeJson = "";
			if (type=="B"){
				_payModeJson = buy_payModeJson;
			}else{
				_payModeJson = sell_payModeJson;
			}
			var _url="";
			if(tag=="S"){
			...
			if("B"==type){
				_curColumns = [ [
					...
					{field:'dealPrice',title:'单价(元)', width:120,editor:{type:'numberbox',options:{required:'true',validType:'length[1,20]',precision:6}},formatter:function(value){if(value){return parseFloat(value).toFixed(6)}}},
					{field:'totalPrice',title:'总价(元)', width:120,editor:{type:'numberbox',options:{required:'true',readonly:'true',validType:'length[1,20]',precision:2}},formatter:function(value){if(value){return parseFloat(value).toFixed(2)}}},
				...
				_curColumns = [ [
					...
					{field:'warehouseName',title:'仓库',width:150},
					{field:'warehousePos',title:'仓库所在地', width:120,align:'left',
						formatter:function(value,row,index){
							return Convert.findTreeValueByKey(_bsAreaJson,"id",value,"text");
					}},
					{field:'warehousePrice',title:'仓储费/地区单价(元)', width:100,align:'right',hidden:'hidden',editor:{type:'numberbox',options:{validType:'length[1,20]',precision:3}}},
...
//					}
			...
			if(entityStatus=="N"|| entityStatus=="B"|| entityStatus=="C"){
				...
				$('#b_payKind'+randomNum).combobox({
				});
				$('#s_payKind'+randomNum).combobox({
				});
				...
				$("#buyCompanyName"+randomNum).combobox({
					url:_ctx+"/bs/company/listMyCompany/"+buy_companyId,
					mode:'remote',
					limitToList:true,
					textField:'text',
					valueField:'text',
					...
					onSelect:function(record){
						...
						if (myFlag!=true){
						    ...
						    if (entityStatus=="B"|| entityStatus=="C"){
						    	var entityBondAmount = $('#payBondAmount'+randomNum).numberbox('getValue');
								var bondTime = $('#payBondTime'+randomNum).datebox('getValue');
								var fullTime = $('#payFullTime'+randomNum).datebox('getValue');
								...
								if (entityBondAmount != null && entityBondAmount != "" && entityBondAmount != undefined){
									for(var i=0;i<_payModeJson.length;i++){
										buy_payModeJson[i].content = buy_payModeJson[i].content.replace('****元', entityBondAmount+'元');
										 if(i != 1){
											 buy_payModeJson[i].content = buy_payModeJson[i].content.replace(sDate, receiveBondTime);
										  }
									}
									buy_payModeJson[1].content = buy_payModeJson[1].content.replace(sDate, receiveFullTime);
								}
						    }
				...
				$("#payBondAmount"+randomNum).numberbox({
					 ...
					 onChange:function(newValue,oldValue){
					   for(var i=0;i<_payModeJson.length;i++){
						   if (oldValue){
							   buy_payModeJson[i].content = buy_payModeJson[i].content.replace(oldValue+'元', newValue+'元');
						   }else{
							   buy_payModeJson[i].content = buy_payModeJson[i].content.replace('****元', newValue+'元');
						   }
						}
					   	var payMode = $('#b_payKind'+randomNum).combobox('getValue');
						$('#b_payKind'+randomNum).combobox('loadData',buy_payModeJson);
						$('#b_payKind'+randomNum).combobox('select',payMode);
					 }
				});
				...
				$("#receiveBondAmount"+randomNum).numberbox({
					 ...
					 onChange:function(newValue,oldValue){
					   for(var i=0;i<_payModeJson.length;i++){
					   	...
					   	var payMode = $('#s_payKind'+randomNum).combobox('getValue');
						$('#s_payKind'+randomNum).combobox('loadData',_payModeJson);
						$('#s_payKind'+randomNum).combobox('select',payMode);
					 }
				});
				...
				// 质量标准
				$('#qualityStandard'+randomNum).combobox({
				...
				$("#payFullTime"+randomNum).datebox({
					...
					onChange : function(newValue,oldValue){
						...
						var payMode = $('#b_payKind'+randomNum).combobox('getValue');
						$('#b_payKind'+randomNum).combobox('loadData',_payModeJson);
						$('#b_payKind'+randomNum).combobox('select',payMode);
					}
				});
				...
				$('#ourCompany_name'+randomNum).combobox({
					...
					onLoadSuccess:function(data){
						...
						var _remark = Convert.findValueByKey(data,"dictName",value,"remark");
						loadData(_remark);
						if (entityStatus=="B"|| entityStatus=="C"){
					    	var entityBondAmount = $('#receiveBondAmount'+randomNum).numberbox('getValue');
							var bondTime = $('#receiveBondTime'+randomNum).datebox('getValue');
							var fullTime = $('#receiveFullTime'+randomNum).datebox('getValue');
					    	var receiveBondTime = formattData(bondTime);
							var receiveFullTime = formattData(fullTime);
							var sDate=formattData(myformatter(new Date()));
							if (entityBondAmount != null && entityBondAmount != "" && entityBondAmount != undefined){
								for(var i=0;i<_payModeJson.length;i++){
									sell_payModeJson[i].content = sell_payModeJson[i].content.replace('****元', entityBondAmount+'元');
								}
								sell_payModeJson[0].content = sell_payModeJson[0].content.replace(sDate, receiveFullTime);
								sell_payModeJson[1].content = sell_payModeJson[1].content.replace(sDate, receiveBondTime);
							}
					    }
					}
				});
				...
				$("#b_payKind"+randomNum).combobox({
					...
					onSelect : function(node){
						if(node.serialNumber == 'QKZF'){
							$('#payBondTime'+randomNum).datebox('options').required=false;
						}else{
				...
				$("#s_payKind"+randomNum).combobox({
					...
					onSelect : function(node){
						if(node.serialNumber == 'QKZF'){
							$('#receiveBondTime'+randomNum).datebox('options').required=false;
						}else{
							$('#receiveBondTime'+randomNum).datebox('options').required=true;
						}
					}
				});
				...
				if("B"==type){
					$("#payBondAmount"+randomNum).numberbox('textbox').on('blur',function(){
						...
						for(var i=0; i<rows.length; i++){
						 ...
						 var bondAmount = Number($("#payBondAmount"+randomNum).numberbox('getValue'));
						 if(bondAmount>totalAmount){
						   	$("#payBondAmount"+randomNum).numberbox('setValue', totalAmount);
						 }
					});
				}else{
					$("#receiveBondAmount"+randomNum).numberbox('textbox').on('blur',function(){
						var _datagrid = $('#relaTable'+randomNum);
						var rows = $('#relaTable'+randomNum).datagrid('getRows');
						var totalAmount = 0;
						var _totalPrice=0;
						for(var i=0; i<rows.length; i++){
						 ...
						 var bondAmount = Number($("#receiveBondAmount"+randomNum).numberbox('getValue'));
						 if(bondAmount>totalAmount){
						   	$("#receiveBondAmount"+randomNum).numberbox('setValue', totalAmount);
						 }
					});
				}
				...
				if(editableFlg){
					if(_businessType=="DL-KZ"){
						if(type=="B"){
							$("#payBondAmount"+randomNum+","+"#warehouseCost"+randomNum+","+"#transportCost"+randomNum+","+"#qingguanFee"+randomNum+","+"#kaizhengFee"+randomNum+","+"#chengduiFee"+randomNum+","+"#chengduiFee"+randomNum).textbox({
								readonly:false,
								min:0,
								precision:2
								//required: true
							});
							$("#arrivalTime"+randomNum+","+"#payBondTime"+randomNum+","+"#payFullTime"+randomNum).datebox({
								height:30,
								readonly:false,
								editable:false
								//required: true
							...
							$("#payBondAmount"+randomNum+","+"#warehouseCost"+randomNum+","+"#transportCost"+randomNum+","+"#qingguanFee"+randomNum+","+"#dailiFee"+randomNum).textbox({
								readonly:false,
								min:0,
								precision:2
								//required: true
							});
							$("#arrivalTime"+randomNum+","+"#receiveBondTime"+randomNum+","+"#receiveFullTime"+randomNum).datebox({
								height:30,
								readonly:false,
								editable:false
								//required: true
