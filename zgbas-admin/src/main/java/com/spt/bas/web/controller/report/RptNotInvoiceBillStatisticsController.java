...
import com.spt.bas.report.client.remote.IRptNotInvoiceBillStatisticsClient;
...
public class RptNotInvoiceBillStatisticsController extends PageController<RptNotInvoiceBillStatistics, BaseVo> {

	@Autowired
	private IRptNotInvoiceBillStatisticsClient rptNotInvoiceBillStatisticsClient;
	@Autowired
	private ICtrContractClient ctrContractClient;
	@Override
	public BaseClient<RptNotInvoiceBillStatistics> getService() {
		return rptNotInvoiceBillStatisticsClient;
	}
	@Autowired
	...
	public String index(Model model,HttpServletRequest request) {
		...
		return "report/notInvoiceBillStatistics";
	}
	 ...
	 * @param response
	 */
	@RequestMapping(value = "findRptNotInvoiceBillStatisticsPage")
	public void findContractFinancePage(RptNotInvoiceBillStatisticsSearchVo searchVo, HttpServletRequest request, HttpServletResponse response) {
		...
		PageDown<RptNotInvoiceBillStatistics> page = rptNotInvoiceBillStatisticsClient.findRptNotInvoiceBillStatisticsPage(searchVo);
		page = preContractData(page);
		JsonEasyUI.renderJson(response, page, null, getFooter(searchVo));
	}
	
	
	public Map<String, Object> getFooter(RptNotInvoiceBillStatisticsSearchVo searchVo){
		Map<String, Object> footer = new HashMap<>();
		RptNotInvoiceBillStatistics sum = rptNotInvoiceBillStatisticsClient.findRptNotInvoiceBillStatisticsSum(searchVo);
		...
		if (Objects.nonNull(sum)) {
			...
			footer.put("invoiceBillAmount", sum.getInvoiceBillAmount());
		}
		return footer;
	}
	...
	@ResponseBody
	public void exportExcel(RptNotInvoiceBillStatisticsSearchVo searchVo, HttpServletRequest request, HttpServletResponse response) throws ApplicationException {
		List<BsDictData> listByCategory = BsDictUtil.getListByCategory(ShiroUtil.getEnterpriseId(), BasConstants.DICT_HG_MATCH_USER_IDS);
		List<Long> hgMatchUserIdList = new ArrayList<>();
		if (CollectionUtils.isNotEmpty(listByCategory)) {
			for (BsDictData bsDictData : listByCategory) {
		...
		String[] titles = new String[]{"合同编号", "我方抬头", "客户名称", "业务类型", "合同数量", "合同总价", "签订日", "已收款金额","收全款日期","已开票金额","开票日期"};
		String[] attrs = new String[]{"sellContractNo", "ourCompanyName", "sellCompanyName","profitType", "totalNumber", "sellTotalAmount",
				"contractTime","receiveAmount","sellPayFullDate","invoiceBillAmount","invoiceBillDate"};
		...
		while (page != null && page.getContent().size() > 0) {
			...
			if (page.hasNext()) {
				...
				page = rptNotInvoiceBillStatisticsClient.findRptNotInvoiceBillStatisticsPage(searchVo);
				start += batchSize;
			} else {
				page = null;
			}
	...
	private PageDown<RptNotInvoiceBillStatistics> preContractData(PageDown<RptNotInvoiceBillStatistics> page){
		if (page != null && page.getContent().size() > 0) {
			for (RptNotInvoiceBillStatistics notInvoiceBillStatistics : page.getContent()) {
				String profitType = notInvoiceBillStatistics.getProfitType();
				...
				notInvoiceBillStatistics.setProfitType(profitTypeText);
			}
		}
		return page;
	}
