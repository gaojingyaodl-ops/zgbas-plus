 ...
 */
@Controller
@RequestMapping(value = "/apply/applyVipMainReceive")
public class ApplyVipMainReceiveController {
    @Resource
    ...
    public String content(@PathVariable("id") Long id, PmPermissionVo permissionVo, Model model,
        ...
        ApplyVipMainReceive entity = getEntity(id, processCode);
        ...
        return "apply/vip-main-receive-content";
    }
    ...
    public ApplyVipMainReceive getEntity(@RequestParam(value = "id", required = false) Long id, @RequestParam(value = "processCode", required = false) String processCode) {
        ApplyVipMainReceive enObject = null;
        if (id != null) {
            if (id > 0 && StringUtils.isNotBlank(processCode)) {
                enObject = (ApplyVipMainReceive) ProcessControlUtil.getEntity(id, processCode);
                if (enObject != null) {
                    enObject.setId(id);
                }
                return enObject;
            } else {
                ApplyVipMainReceive applyVipReceive = new ApplyVipMainReceive();
