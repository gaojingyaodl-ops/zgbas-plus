...
public class ApplyAgreementVirtualController extends PageController<ApplyAgreementVirtual, BaseVo> {
    @Resource
    ...
    @Override
    public BaseClient<ApplyAgreementVirtual> getService() {
        return applyAgreementVirtualClient;
    }

    @ModelAttribute("preload")
    public ApplyAgreementVirtual getEntity(@RequestParam(value = "id", required = false) Long id) {
        ApplyAgreementVirtual entity = new ApplyAgreementVirtual();
        entity.setId(0L);
        if (id != null) {
            if (id == 0L) {
    ...
    public void updateFileId(FileIdUpdateVo vo,
        ...
        try {
            applyAgreementVirtualClient.updateFileId(vo);
