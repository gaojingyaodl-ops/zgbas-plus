 ...
 */
@Controller
@RequestMapping(value = "/apply/applyDeposit")
public class ApplyBsCompanyDepositController extends PageController<ApplyDeposit, BaseVo> {
    @Override
    public BaseClient<ApplyDeposit> getService() {
        return null;
    }
    @Resource
    ...
    public String content(@PathVariable("id") Long id, PmPermissionVo permissionVo, Model model, HttpServletRequest request) {
        ...
        ApplyDeposit entity = getEntity(id, processCode);
        ...
        return "apply/applyDeposit";
    }

    @ModelAttribute("preload")
    public ApplyDeposit getEntity(@RequestParam(value = "id", required = false) Long id,
                                      @RequestParam(value = "processCode", required = false) String processCode) {
        ApplyDeposit enObject = null;
        if (id != null) {
            if (id > 0 && StringUtils.isNotBlank(processCode)) {
                enObject = (ApplyDeposit) ProcessControlUtil.getEntity(id, processCode);
                enObject.setId(id);
                return enObject;
            } else {
                ApplyDeposit bsCompanyAllowed = new ApplyDeposit();
