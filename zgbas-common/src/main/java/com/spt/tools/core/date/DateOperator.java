...
public class DateOperator extends DateParser {
	...
	public static int getMonthes(Date start, Date end) {
		if (start.after(end)) {
			Date t = start;
			start = end;
			end = t;
	...
	public static int compareMonthes_1(Date startDate, Date endDate) {
		BigDecimal monthes = BigDecimal.ZERO;
		if (startDate != null && endDate != null) {
			monthes = BigDecimal.valueOf((endDate.getTime() - startDate.getTime()) / 1000f / 60f / 60f / 24f / 31f);
		}
	...
	public static Long compareWeek(Date startDate, Date endDate) {
		...
		if (startDate != null && endDate != null) {
			days = BigDecimal.valueOf((endDate.getTime() - startDate.getTime()) / 1000 / 60 / 60 / 24 / 7);
		}
