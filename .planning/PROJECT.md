...
**当前状态：** v1.0 shipped（2026-07-20）— 7 阶段 37 plans 交付完成，37/37 需求 validated。单体单服务跑全功能（登录→核心业务→报表→定时任务），ALIGN-01/02 sign-off。
...
- basWx 微信采购小程序迁入（WX-01, WX-02）— v1.0 已 deferred
- Phase 6 28 生产 handler quartz 路由 gap-closure
...
- [ ] basWx 微信采购小程序迁入（WX-01, WX-02）— 11 实体 + Dao + purchase-client 契约内联
- [ ] Phase 6 28 生产 handler quartz 路由 gap-closure
- [ ] Phase 4 entity schema drift 修复（ddl-auto=none → validate）
- [ ] 启动测试 hermetic 化
- [ ] 生产库明文密码轮换
