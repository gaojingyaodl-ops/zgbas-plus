...
- ✓ 合同 / 授信 / 库存 / 放款等核心供应链业务（源：basServer，JPA）— *Validated in Phase 4（service/impl/域子包 533 + PM 88 + infra + api 236 + BFF 267 迁入；D-P4-01 方案 A Feign 自回环 + WR-02 HTTP proof；compile 0 ERROR + 启动 20 绿；约束 #9/#10 持守）*
...
- ✓ 64 个定时任务（合同调度、自动盖章、结算、数据同步等，源 xxl-job）— *Validated in Phase 6（xxl-job 移除；32 handler 类 / ~60 @XxlJob 方法迁入 zgbas-quartz；53 sys_job 行从生产 xxl_job_info 翻译落库；/monitor/job 可视化运维台；D-P6-06 启动 fail-fast + dry-run 通过；⚠ 28 生产 handler 待 gap-closure，见 deferred-items）*
...
- [x] 微服务 4 服务 → **单体 1 服务**，单进程启动跑全功能 — *Validated in Phase 7（ALIGN-01 全链路端到端可达：登录→首页→核心业务→报表→定时任务；ALIGN-02 行为等价：9/9 UAT PASS + 5 @Disabled proof 全绿）*
- [x] 打包从 4 可执行 jar → **少数 jar**（仅 zgbas-admin 产可执行 fat jar）— *Validated in Phase 1*
- [x] 5 模块聚合单体：`zgbas-admin / zgbas-common / zgbas-framework / zgbas-quartz / zgbas-system` — *Validated in Phase 1*
- [x] **双 ORM 兼容**：JPA 增删改查主力 + mybatis 复杂报表，单数据源共存 — *Validated in Phase 2（@Primary Druid DataSource，239 实体 + 240 Dao + sample Mapper，JPA 审计链完整）*
- [x] `spt-tools-*` 代码**内联**进 zgbas-common（摆脱私服 jar 依赖）— *Validated in Phase 2（10 模块全量内联 172 类，零 spt-tools-* 声明）*
- [x] 外部服务 Bean（AuthOpenFacade / PushClientHttp / FileRemote / CfcaSignClient）**保持原 HTTP 注入方式** — *Validated in Phase 2（3 bean @Bean 注入，secret log 已删，cfca Feign 收窄）*
- [x] **删除 nacos**（服务发现 + 相关配置）— *Validated in Phase 2（依赖图 + 配置 0 nacos）*
- [x] **xxl-job → RuoYi quartz**（zgbas-quartz 模块）— *Validated in Phase 6（06-01..06-06；xxl-job-core 删除 + 60 handler 迁入 + sys_job 数据翻译 + /monitor/job UI + 启动 fail-fast；28 生产 handler 路由 gap-closure）*
- [x] 登录接口 + 首页**完全照搬**旧项目实现 — *Validated in Phase 3（Shiro session+cookie 链路 + Realm/Service/Util + Login/Index/SSO Controller + Thymeleaf + 608 模板/2097 静态资源；D-P3-13 启动验证非 hermetic，按 Option 4 本地 export 密钥即通过）*
...
*Last updated: 2026-07-19 after Phase 6 completion*
