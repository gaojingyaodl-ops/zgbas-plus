 ...
 * 同步老审批和数据中台合同表数据定时任务
 *
 * <p>Phase 6 (06-03) — ported from {@code com.spt.bas.server.rocketmq.task.SynchronizedCtrProductTask}.
 * Bean name {@code "synchronizedCtrProductTask"} aligns with {@code sys_job.invoke_target}
 * short name {@code synchronizedCtrProductTask.synchronizedAllCtrProduct}. Per D-P6-11,
 ...
 */
@Component("synchronizedCtrProductTask")
public class SynchronizedCtrProductTask {
    ...
    class ThreadQuery implements Runnable {
        ...
        @Override
        public void run() {
            getCtrProduct(this.pageIndex, this.perPageCount);
        }
